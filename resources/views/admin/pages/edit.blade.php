            ...
            <form method="POST" action="{{ route('admin.pages.update', $page->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control @error('slug') is-invalid @enderror" 
                                       ...
                                       {{ in_array($page->slug, ['about-us', 'contact-us']) ? 'readonly' : '' }}>
                            </div>
                            <small class="form-text text-muted">
                                @if(in_array($page->slug, ['about-us', 'contact-us']))
                                    This is a system page with a fixed slug.
                                @else
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control summernote @error('content') is-invalid @enderror" 
                              id="content" name="content" rows="8" required>{{ old('content', $page->content) }}</textarea>
                    @error('content')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
...
<script>
    $(document).ready(function() {
        ...
        // Auto-generate slug from title (only if not a system page)
        @if(!in_array($page->slug, ['about-us', 'contact-us']))
        $('#title').on('blur', function() {
            const titleVal = $(this).val();
            const slugField = $('#slug');
            
            // Only auto-generate if slug field is empty or matches the previous auto-generation
        ...
        $('#remove_image').on('change', function() {
            if($(this).is(':checked')) {
                $('#featured_image').prop('disabled', true);
            } else {
                $('#featured_image').prop('disabled', false);
            }
        });
    });
</script>
